...
public class MainMenuManager : SceneSingleton<MainMenuManager>
{
    ...
    [Header("UI Panels")] [SerializeField] private GameObject mainTitlePanel; // 메인 패널
    ...
    [Header("아바타 토글 그룹")] public Toggle toggleMaleChar;
    ...
    public bool SaveInitGameData(PlayerModelType selectedType)
    {
        ...
        if (playerService.CreatePlayer(selectedType))
        {
            ...
            if (encounterService.CreateEncounters())
            {
                ...
                if (inventoryService.InitialInventoryAndEquipment())
                {
                    // 스토어에 기본 상점 아이템 저장하기
                    StoreService storeService = new StoreService();
                    storeService.InitialStore();
